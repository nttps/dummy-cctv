<template>
  <div class="flex justify-left text-black text-xl ml-52 mt-5">
        <UButton color="blue" size="xl" variant="solid" @click="navigateTo('/station-management/create')">Add Station</UButton>
...
>
    ...
    <UTable
      ...
      <template #status-data="{ row }" >
        <UBadge size="xs" :label="row.status ? 'ออนไลน์' : 'ออฟไลน์'" :color="row.status ? 'green' : 'red'" variant="subtle" />
      </template>

      <template #actions-data="{ row }" >
        <UButton  color="purple" size="xl" variant="solid" :to="`/station-management/${row.code}`"><span class="mr-2">
...
const { data: stations, pending } = await useLazyAsyncData<{
  ...
  name: string
  code: string
}[]>('stations', () => ($fetch as any)(`/api/v1/stations`, {
  query: {
    q: search.value,
    '_page': page.value,
